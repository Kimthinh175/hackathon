  ...
  return (
    <section id="assets" className="view-section animate-up">
      <div className="page-header mb-12">
        <div>
          <h2 className="text-3xl font-black text-gray-800 tracking-tight">Quản lý Tài sản Cố định</h2>
          <p className="text-gray-400 font-bold text-sm tracking-widest uppercase mt-1">Truy vết khấu hao & Lịch sử cấp phát thiết bị</p>
        </div>
        <button className="bg-indigo-600 text-white px-8 py-3 rounded-2xl font-black text-sm shadow-xl hover:bg-indigo-700 hover:-translate-y-1 transition duration-300">
            <i className="fa-solid fa-file-invoice-dollar mr-2"></i> BÁO CÁO KHẤU HAO
        </button>
      </div>
                ...
                return (
                  <tr key={asset._id} className="hover:bg-indigo-50/20 transition-colors group">
                    ...
                    <td className="py-6 px-8 text-right">
                        {isInUse ? (
                           <button onClick={() => handleAction(asset._id, 'RETURN')} className="bg-rose-50 text-rose-500 px-6 py-2.5 rounded-xl font-black text-[10px] hover:bg-rose-600 hover:text-white transition shadow-sm uppercase tracking-widest">Thu hồi</button>
                        ) : (
                           <button onClick={() => handleAction(asset._id, 'ALLOCATE')} className="bg-emerald-50 text-emerald-600 px-6 py-2.5 rounded-xl font-black text-[10px] hover:bg-emerald-600 hover:text-white transition shadow-sm uppercase tracking-widest">Cấp phát</button>
                        )}
                    </td>
                  </tr>
                )
